...
export default function Watchlist({ onSidebarHide }) {
        ...
        <div className="w-full sm:flex p-2 items-end">
          <div className="sm:flex-grow flex flex-wrap justify-between">
            <div className="text-3xl font-bold text-white">
              {user?.loginStatus ? user.user.name : "Login!"}'s watchlist
            </div>
...
function TopCountries({ email }) {
  ...
  const top_stock_data = async () => {
    await axios
      .get(`http://localhost:5000/api/watchlist/getWatchlist/${email}`)
      .then((res) => {
  ...
  const handelDelete = async (symbol) => {
    setLoading(true);
    await axios
      .post("http://localhost:5000/api/watchlist/deleteStock", {
        symbol: symbol,
        email: email,
      })
      .then(async (res) => {
